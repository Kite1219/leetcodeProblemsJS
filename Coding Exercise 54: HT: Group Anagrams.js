...
function groupAnagrams(arr) {
  ...
  for (let string of arr) {
    const canonical = Array.from(string).sort().join("");
...
function groupAnagrams(strings) {
  ...
  for (const string of strings) {
    const canonial = Array.from(string).sort().join("");
    if (anagramGroups[canonial]) {
      anagramGroups[canonial].push(string);
    } else {
      const group = [string];
      anagramGroups[canonial] = group;
    }
  }
